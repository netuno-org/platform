...
val database = mongo.database("product_db")

database.createCollection("product")

val collection = database.collection("product")
...
val laptopId = collection.insertOne(
  _val.map()
    .set("name", "Laptop")
    .set("quantity", 22)
    .set("price", 100)
...
_out.println("<ul>")
collection.find().all().forEach { doc ->
...
val oldLaptop = collection.findOneAndUpdate(
  _mongo.filters().eq("name", "Laptop"),
  _mongo.updates().set("quantity", 42)
)
...
val oldTablet = collection.findOneAndReplace(
...
val computers = collection.countDocuments(
  _mongo.filters().eq("category", "computers")
)
...
val removed = collection.findOneAndDelete(
  _mongo.filters().eq("name", "Tablet")
)
...
val bsonDoc = _mongo.valToDoc(
  _val.map()
    .set("name", "Laptop")
    .set("quantity", 22)
)

val valuesFromBson = _mongo.docToVal(bsonDoc)
