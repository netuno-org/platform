...
database = mongo.database("product_db")
...
_out.println("<p>Inserted id: "+ str(laptopId) +"</p>")
...
oldLaptop = collection.findOneAndUpdate(
  _mongo.filters().eq("name", "Laptop"),
  _mongo.updates().set("quantity", 42)
)
...
oldTablet = collection.findOneAndReplace(
...
computers = collection.countDocuments(
  _mongo.filters().eq("category", "computers")
)

_out.println("<p>Total: "+ str(collection.countDocuments()) +"</p>")
_out.println("<p>Computers: "+ str(computers) +"</p>")
_out.println("<p>Estimated: "+ str(collection.estimatedDocumentCount()) +"</p>")
...
removed = collection.findOneAndDelete(
  _mongo.filters().eq("name", "Tablet")
)
...
bsonDoc = _mongo.valToDoc(
  _val.map()
    .set("name", "Laptop")
    .set("quantity", 22)
)

valuesFromBson = _mongo.docToVal(bsonDoc)
...
_out.println("<p>Remaining documents: "+ str(collection.countDocuments()) +"</p>")
