...
(function() {
    ...
    if (validation === false || form.validate().valid()) {
      ...
      form.ajaxForm({
        ...
        success: function(response) {
          ...
          container.html(response);
          netuno.contentLoaded(container);
          return netuno.loadValidationDev(formId);
        }
      }).submit();
      return true;
    } else {
  ...
  netuno.componentConfig = {
    load: function(netunoType, netunoUid) {
      var NetunoType, componentType, fieldUid, netunoContainer, netunoName;
      NetunoType = '';
      if (netunoType === 'form') {
        NetunoType = 'Form';
      }
      ...
      return $(`\#field${NetunoType}Design_${netunoName}_componentConfiguration`).load(`${netuno.config.urlBase}dev/ComponentConfiguration${netuno.config.extension}?component=${componentType}&uid=${fieldUid}&netuno_table_uid=${netunoUid}`);
    },
    link: {
      popup: null,
      callbackItem: null,
      ...
      contentLoaded: function() {
        ...
        if ($('.component_config_link_popup_field_item').length) {
          netuno.componentConfig.link.popup.find("[data-netuno-back]").off('click').on('click', function() {
            var back;
            back = $(this);
            return netuno.componentConfig.link.popup.find(".modal-body").empty().load(`${netuno.config.urlBase}dev/Link${netuno.config.extension}?mode=${back.attr('data-mode')}&netuno_table_uid=${back.attr('data-table-uid')}&parameter_key=${back.attr('data-parameter-key')}`, netuno.componentConfig.link.contentLoaded);
          }).show();
      ...
      openTable: function(mode, netunoTableUid, parameterKey, tableUid) {
        return netuno.componentConfig.link.popup.find(".modal-body").empty().load(`${netuno.config.urlBase}dev/Link${netuno.config.extension}?mode=${mode}&netuno_table_uid=${netunoTableUid}&parameter_key=${parameterKey}&table_uid=${tableUid}`, netuno.componentConfig.link.contentLoaded);
      },
