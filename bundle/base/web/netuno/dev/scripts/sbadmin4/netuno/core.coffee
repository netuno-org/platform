  ...
  buildMenu = (type, parentUId, items, level) ->
    ...
    for item in items
      ...
      if item.items.length > 0
        expand = " data-toggle"
      menuHtml += "<li class=\"nav-item\" netuno-menu-#{ type }-item>"
      menuHtml += "<a class=\"nav-link\" href=\"\#\" netuno-dev-#{ type }=\"#{ item.name }\"#{ expand }>"
      menuHtml += "#{ item.text }"
      if item.items.length > 0
...
netuno.loadDevMenu = () ->
  $.ajax(
    type: 'POST'
    url: "#{ netuno.config.urlBase }dev/Main#{ netuno.config.extension }"
    data: { 'service': 'json' }
    success: (response) ->
...
netuno.loadDevForm = (name) ->
  ...
  if container.is(':empty')
    $.ajax(
      type: 'POST'
      url: "#{ netuno.config.urlBase }dev/FormDesign#{ netuno.config.extension }"
...
netuno.loadDevReport = (name) ->
  ...
  if container.is(':empty')
    $.ajax(
      type: 'POST'
      url: "#{ netuno.config.urlBase }dev/ReportDesign#{ netuno.config.extension }"
...
netuno.componentConfig = {
  load: (netunoType, netunoUid)->
    ...
    $("\#field#{ NetunoType }Design_#{ netunoName }_componentConfiguration").load("#{ netuno.config.urlBase }dev/ComponentConfiguration#{ netuno.config.extension }?component=#{ componentType }&uid=#{ fieldUid }&netuno_table_uid=#{ netunoUid }");
  link: {
    popup: null,
    callbackItem: null,
    select: (netunoTableUid, callbackItem)->
      ...
      netuno.componentConfig.link.popup.find(".modal-body").empty().load("#{ netuno.config.urlBase }dev/Link#{ netuno.config.extension }?mode=select", netuno.componentConfig.link.contentLoaded)
      netuno.componentConfig.link.popup.find("[data-netuno-back]").hide()
      netuno.componentConfig.link.popup.modal("show")
    ,
    add: (netunoTableUid, parameterKey, callbackItem)->
      netuno.componentConfig.link.callbackItem = callbackItem
      ...
      netuno.componentConfig.link.popup.find(".modal-body").empty().load("#{ netuno.config.urlBase }dev/Link#{ netuno.config.extension }?mode=add&netuno_table_uid=#{ netunoTableUid }&parameter_key=#{ parameterKey }&table_uid=#{ tableUid }", netuno.componentConfig.link.contentLoaded)
      netuno.componentConfig.link.popup.modal("show")
    ,
    remove: (netunoTableUid, parameterKey, callbackItem)->
      ...
      netuno.componentConfig.link.popup.find(".modal-body").empty().load("#{ netuno.config.urlBase }dev/Link#{ netuno.config.extension }?mode=remove&netuno_table_uid=#{ netunoTableUid }&parameter_key=#{ parameterKey }&table_uid=#{ tableUid }", netuno.componentConfig.link.contentLoaded)
      netuno.componentConfig.link.popup.modal("show")
    ,
    contentLoaded: ()->
      if $('.component_config_link_popup_table_item').length
        ...
        $('.component_config_link_popup_table_item').on 'click', (e)->
          ...
          e.stopPropagation()
          item = $(this)
          netuno.componentConfig.link.openTable(item.attr('data-mode'), item.attr('data-netuno-table-uid'), item.attr('data-parameter-key'), item.attr('data-table-uid'))
      if $('.component_config_link_popup_field_item').length
        netuno.componentConfig.link.popup.find("[data-netuno-back]").off('click').on('click', ()->
          back = $(this);
          netuno.componentConfig.link.popup.find(".modal-body").empty().load("#{ netuno.config.urlBase }dev/Link#{ netuno.config.extension }?mode=#{ back.attr('data-mode') }&netuno_table_uid=#{ back.attr('data-table-uid') }&parameter_key=#{ back.attr('data-parameter-key') }", netuno.componentConfig.link.contentLoaded)
        ).show()
    ...
    openTable: (mode, netunoTableUid, parameterKey, tableUid)->
      netuno.componentConfig.link.popup.find(".modal-body").empty().load("#{ netuno.config.urlBase }dev/Link#{ netuno.config.extension }?mode=#{ mode }&netuno_table_uid=#{ netunoTableUid }&parameter_key=#{ parameterKey }&table_uid=#{ tableUid }", netuno.componentConfig.link.contentLoaded)
    ,
